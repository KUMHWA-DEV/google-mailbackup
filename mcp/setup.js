...
// Windows의 Claude Desktop/Cursor는 npx.cmd 를 직접 띄우지 못하므로 cmd /c 로 감싼다.
const entry = process.platform === 'win32'
  ? { command: 'cmd', args: ['/c', 'npx', '-y', '-p', PKG, 'mailbackup-mcp'], env }
  : { command: 'npx', args: ['-y', '-p', PKG, 'mailbackup-mcp'], env };
...
function upsertJson(file, mutate) {
  let obj = {};
  if (fs.existsSync(file)) { fs.copyFileSync(file, file + '.bak'); try { fs.chmodSync(file + '.bak', 0o600); } catch (e) { /* windows */ } try { obj = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e) { obj = {}; } }
  mutate(obj);
...
for (const app of apps) {
  if (app === 'claude-desktop') {
    ...
    try { execFileSync('claude', ['mcp', 'remove', 'mail-backup', '-s', 'user'], { stdio: 'ignore', shell: process.platform === 'win32' }); } catch (e) { /* 없으면 무시 */ }
    const argv = ['mcp', 'add', 'mail-backup', '-s', 'user'].concat(SCRIPT_ID ? ['-e', 'MAIL_BACKUP_SCRIPT_ID=' + SCRIPT_ID] : []).concat(['--'], entry.command === 'cmd' ? ['cmd', '/c', 'npx'] : ['npx'], ['-y', '-p', PKG, 'mailbackup-mcp']);
    execFileSync('claude', argv, { stdio: 'inherit', shell: process.platform === 'win32' });
