...
## 3-1. 예전 .eml 백업 가져오기 (📥 가져오기 탭)

백업과 **독립된 작업**이다(자체 트리거·상태·인덱스 시트 `Import`, 저장 폴더 `가져옴/`·`_attachments_import/`). 백업과 동시에 돌아도 된다.

1. 각자 드라이브의 `Mail Backup/_import` 폴더(탭에 링크)에 .eml 파일을 넣는다. 하위 폴더째 가능하며 첫 단계 하위 폴더 이름이 라벨(폴더)이 된다. 루트의 파일은 받은편지함(내가 보낸 것은 보낸편지함).
2. 가져오기 탭에서 ▶ 가져오기 시작. 서버가 4분 30초 구간씩 처리하고 이어서 실행한다(컴퓨터 꺼도 됨). 중지/이어서/취소, 진행률, 이력이 있다.
3. 끝나면 메일함 라벨 `가져옴`으로 모아 볼 수 있다. 처리한 원본은 인덱스에 `src:<fileId>`로 기록돼 다시 처리되지 않으며 폴더는 비워도 된다.

파서(`src/lib/mime.js`): multipart, base64/quoted-printable, EUC-KR 등 문자셋, RFC 2047/2231 헤더. 30MB 초과 파일은 오류로 건너뛴다. Message-ID로 중복을 거른다. 앱은 `drive.readonly`로 파일을 읽기만 하므로 원본을 옮기거나 지우지 않는다.
