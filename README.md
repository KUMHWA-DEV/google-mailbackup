...
## 3-1. 예전 백업 가져오기 (📥 가져오기 탭)

백업과 **독립된 작업**이다(자체 트리거 `runImport`·임대 잠금·상태, 인덱스 스프레드시트의 `Import` 시트). 백업과 동시에 돌아도 된다.

1. 각자 드라이브의 `Mail Backup/_import` 폴더(탭에서 "폴더 만들기"/"폴더 열기")에 파일을 넣는다. `.eml`(30MB까지), `.mbox`(크기 제한 없음, Drive API 범위 읽기로 8MB씩 처리), `.mbox.gz`(40MB까지). 하위 폴더는 정리용일 뿐 분류에 쓰지 않는다.
...
파서(`src/lib/mime.js`): multipart, base64/quoted-printable, EUC-KR 등 문자셋, RFC 2047/2231 헤더, mbox 경계·`>From` 해제. Message-ID로 중복을 거른다. 처리한 파일은 `Import` 시트에 `src:<fileId>`(표시 행)로 기록돼 다시 처리되지 않으며, 앱은 `drive.readonly`로 읽기만 하므로 원본을 옮기거나 지우지 않는다.
