...
- 웹앱 (4개 탭, `gstudio/` 레퍼런스의 기능 의도를 따르되 설명 문구는 최소화하고 아이콘·상태 표시로 대체)
  - **대시보드**: 상태 한 줄(최신/백업 필요/진행 중 + 다음 예정일), 주기 진행률, 자동 백업 스위치, "지금 백업"(확인 모달),
    보관 메일(수신·발신·첨부)·용량·보관 기간·최근 실행 타일, 최근 실행 3건
  - **메일함**: Gmail 화면 구성을 그대로 따름 — 상단 검색바, 좌측 내비(전체/받은편지함/보낸편지함/
    중요/임시보관함/첨부파일, 라벨·Drive 폴더, 안건별 분류, 백업 회차), 가운데 메일 행 목록(별표, 보낸사람, 라벨 칩,
    제목 – 요약, 📎, 날짜, 50건 페이지), 우측 읽기창(제목·라벨 칩, 아바타, 보낸/받는 사람 펼침, 본문, 첨부 카드).
    **첨부파일은 카드의 ⬇ 버튼으로 바로 다운로드**되고(백업 시 저장한 Drive 파일 ID 사용), 원본 .eml도 상단 ⬇로 받음
  - **설정**: 주기(1~30일), 시작일(첫 백업만), 회당 최대(0=무제한), 스위치 3개(보낸편지함 포함·첨부 별도 저장·완료 알림),
