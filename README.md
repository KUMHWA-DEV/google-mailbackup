...
각자 드라이브의 `Mail Backup/_import` 폴더(설정 탭에 링크)에 .eml 파일을 넣어 두면(하위 폴더째 가능) 다음 백업 실행이 자동으로 처리한다. 컴퓨터가 꺼져 있어도 트리거가 돈다.

- 서버가 .eml을 직접 해석한다(`src/lib/mime.js`: multipart, base64/quoted-printable, EUC-KR 등 문자셋, RFC 2047/2231 헤더). 일반 백업과 같은 구조로 저장되고 인덱스에 라벨 `가져옴`이 붙는다.
- 분류: 보낸사람이 나면 보낸편지함, 아니면 첫 하위 폴더 이름(없으면 받은편지함). Message-ID로 중복을 거른다.
- 앱은 `drive.readonly`로 파일을 읽기만 하므로 원본을 옮기거나 지우지 않는다. 처리한 파일은 인덱스에 `src:<fileId>`로 기록돼 다시 처리되지 않으며, 폴더는 언제든 비워도 된다. 30MB가 넘는 파일은 건너뛴다.
- "지금 백업" 감지 결과에 "📥 가져올 .eml N건"이 함께 표시된다.
