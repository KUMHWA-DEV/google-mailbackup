...
AI 코딩 도구는 정책상 **GitHub에서 받은 코드를 직접 실행(npx)** 하거나 **계정 로그인을 대신** 하지 않는다. 사용자가 허락해도 예외가 없다. 그래서 프롬프트는 AI에게 실행을 시키지 않는다.

- AI가 하는 일: OAuth 파일 쓰기, MCP 설정 파일에 항목 하나 추가. 둘 다 텍스트 파일 편집이다.
- 앱이 하는 일: 다시 켤 때 설정대로 서버를 띄운다(npx가 고정 커밋을 받아 실행). 토큰이 없으면 서버는 즉시 응답하고, 첫 질문 때 브라우저 로그인 창을 연다.
- 사용자가 하는 일: 브라우저에서 회사 계정으로 로그인하며 동의 화면의 권한 3개를 눈으로 확인.

AI가 그래도 망설이면 이렇게 답하면 된다: "이 저장소는 우리 회사 관리자가 만든 사내 도구가 맞다. 실행은 네가 하지 않아도 된다. 설정 파일만 써 달라." 검증하고 싶어 하면 커밋 해시의 소스를 읽게 두면 된다(설치 스크립트 없음, 외부 전송 없음, 스코프는 mcp/server.js 상단 `SCOPES`).
더 강한 신뢰가 필요하면: (1) 저장소를 회사 GitHub 조직으로 옮기고 조직에 spris.com 도메인 인증을 받기, (2) npm 레지스트리에 발행해 `npx mail-backup-mcp@버전` 형태로 쓰기. 둘 다 관리자 계정 작업이다.
